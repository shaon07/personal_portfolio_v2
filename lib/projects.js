...
// Static import so the data is bundled into the serverless function and is
// always available for reads, even on a read-only filesystem (e.g. Vercel).
...
const readAll = () => {
  try {
    ...
    // The file may not be present on a serverless/read-only filesystem.
    // Fall back to the copy bundled at build time.
    return JSON.parse(JSON.stringify(seedData));
  }
};

class ReadOnlyStorageError extends Error {
  constructor() {
    super(
      'Storage is read-only in this environment. Project writes require a ' +
        'persistent database — see the deployment notes.'
    );
    this.readOnly = true;
  }
}
...
const writeAll = (projects) => {
  try {
    ...
    // EROFS / EACCES on read-only serverless filesystems.
...
export const createProject = (input) => {
  ...
  try {
    ...
    return { error: err.message, readOnly: !!err.readOnly };
  }
  return { project };
};
